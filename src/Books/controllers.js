...
const getBookById = async (req, res) => {
  try {
    ...
    if (checkIdLength(id)) {
      const libro = await LibrosServices.getOneById(id);
      ...
      res
        .status(400)
        .json({ msg: "El id no es correcto. Deberia tener 24 caracteres" });
    }
  } catch (error) {
    debug(error);
    Response.error(res, error);
...
const getBooksByField = async (req, res) => {
